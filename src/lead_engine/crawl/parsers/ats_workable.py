...
def parse_ats_board(url: str, html: str, keywords_config: Dict = None) -> Dict:
    ...
    try:
        ...
        # Workable uses specific selectors
        job_elements = []
        
        # Try Workable-specific selectors
        selectors = [
            "div.job",
            "a.job-title",
            "[data-job-id]",
        ...
        for element in job_elements:
            ...
            # Try to find title in Workable structures
        ...
        logger.debug("Parsed Workable board", url=url, jobs=len(job_titles), engineering=engineering_count)
        return result
        
    except Exception as e:
        logger.error("Error parsing Workable board", url=url, error=str(e), exc_info=True)
        return {
            "jobs_count": 0,
