...
def parse_ats_board(url: str, html: str, keywords_config: Dict = None) -> Dict:
    ...
    try:
        ...
        for element in job_elements:
            ...
            # Try to find title in Lever structures
            title_elem = element.find(class_=re.compile(r"posting-title|job-title|title", re.I))
            if title_elem:
        ...
        logger.debug("Parsed Lever board", url=url, jobs=len(job_titles), engineering=engineering_count)
        return result
        
    except Exception as e:
        logger.error("Error parsing Lever board", url=url, error=str(e), exc_info=True)
        return {
            "jobs_count": 0,
