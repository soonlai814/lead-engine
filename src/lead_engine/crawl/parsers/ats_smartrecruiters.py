"""SmartRecruiters ATS board parser."""
...
def parse_ats_board(url: str, html: str, keywords_config: Dict = None) -> Dict:
    """
    Parse SmartRecruiters ATS board HTML.
    ...
    try:
        ...
        # SmartRecruiters uses specific selectors
        job_elements = []
        
        # Try SmartRecruiters-specific selectors
        selectors = [
            "div.job",
            "a.job-title",
            "[data-job-id]",
        ...
        for element in job_elements:
            ...
            # Try to find title in SmartRecruiters structures
        ...
        logger.debug("Parsed SmartRecruiters board", url=url, jobs=len(job_titles), engineering=engineering_count)
        return result
        
    except Exception as e:
        logger.error("Error parsing SmartRecruiters board", url=url, error=str(e), exc_info=True)
        return {
            "jobs_count": 0,
